...
const say = (s) => { $('notice').textContent = s; log(s); };
const modeNames = ['六轴', '九轴', '九轴相对角'];
function setConnected(v) { connected = v; $('connect').disabled = v; $('disconnect').disabled = !v; $('dot').className = v ? 'on' : ''; $('link').textContent = v ? '已连接' : '未连接'; if (!v) { setting = false; updateSettings(); } }
function updateSettings() { $('settings').textContent = setting ? '设置模式已进入' : '未进入设置模式'; $('settings').className = `pill ${setting ? 'on' : ''}`; $('modeGroup').disabled = !setting; $('apply').disabled = !setting; $('exit').disabled = !setting; $('enter').disabled = setting; }
async function send(cmd, payload = []) { if (!connected) { say('请先连接串口'); return false; } try { const data = packet(cmd, seq++, payload); await window.host.write(Array.from(data)); log(`TX 0x${cmd.toString(16).padStart(2,'0')} ${payload.map(x=>x.toString(16).padStart(2,'0')).join(' ')}`); return true; } catch(e) { say(`发送失败：${e.message}`); return false; } }
...
$('apply').onclick=async()=>{const immediate=$('restart').checked;pendingReset=immediate;pendingExit=!immediate;await send(CMD.MODE,[Number(document.querySelector('input[name="mode"]:checked').value),immediate?1:0]);say('已发送模式设置，等待 ACK');};
$('setCan').onclick=async()=>{if(!setting){say('请先进入设置模式');return} const n=Number($('canId').value);if(!Number.isInteger(n)||n<0||n>0x7ff){say('CAN ID 必须为 0~0x7FF');return}await send(CMD.CAN_ID,[n&255,n>>8]);};
