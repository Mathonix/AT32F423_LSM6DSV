#!/usr/bin/env python3
"""Build/flash the non-destructive selectable-mode SPI frequency sweep."""
...
HEX_PATH = ROOT / "build" / "spi_freq_sweep.hex"
SRAM = 0x20000000
SRAM_SIZE = 48 * 1024
MAGIC = 0x5350494D
HEADER_FMT = "<16I"
...
DIVISORS = [1024, 512, 256, 128, 64, 32, 16, 8, 4, 2]
TIMINGS = [100, 10, 2]


def build(mode: int, exact_10mhz: bool) -> None:
    extra = f"-DSPI_SWEEP_MODE={mode}"
    if exact_10mhz:
        extra += (
            " -DSPI_SWEEP_USE_PLL=1"
            " -DSYSTEM_CLOCK_TARGET_HZ=80000000U"
            " -DSYSTEM_APB2_DIV_VALUE=1U"
        )
    label = "nominal 10 MHz sweep" if exact_10mhz else "low-frequency sweep"
    print(f"building spi_freq_sweep firmware (mode {mode}, {label}) ...")
    subprocess.run(
        ["make", "-B", "spi-freq-sweep", f"EXTRA_CFLAGS={extra}"],
        cwd=ROOT, check=True,
    )
...
def write_csv(rows: list[dict]) -> Path:
    ...
    path = ROOT / "build" / f"spi_freq_sweep_results_{stamp}.csv"
...
def summarize(header: dict, rows: list[dict]) -> None:
    ...
    if success:
        ...
        print("\nNo safe hardware-SPI configuration passed 100/100.")
...
def main() -> int:
    ...
    p.add_argument(
        "--exact-10mhz", action="store_true",
        help="run from nominal 80 MHz PLL/APB2 and sweep /1024 through /8 (10 MHz)",
    )
    args = p.parse_args()

    if not args.no_build:
        build(args.mode, args.exact_10mhz)
    ...
    try:
        ...
        while True:
            ...
            if h["state"] in (2, 3):
                break
            if time.time() >= deadline:
                raise TimeoutError(f"safe probe did not finish, count={h['count']}")
            time.sleep(1.0)
        ...
        if not rows:
            ...
            if h["fatal_code"] == 0x10:
                ...
                print("The read-only precheck failed; leave the board parked and inspect power/SPI.")
            return 2
        path = write_csv(rows)
        summarize(h, rows)
        print(f"\nCSV: {path}")
