#!/usr/bin/env python3
"""Build/flash the non-destructive Mode-0 SPI comparison probe and print SRAM results."""
...
HEX_PATH = ROOT / "build" / "spi_safe_probe.hex"
SRAM = 0x20000000
SRAM_SIZE = 48 * 1024
MAGIC = 0x5350494D
HEADER_FMT = "<16I"
...
def build() -> None:
    print("building spi_safe_probe firmware ...")
    subprocess.run(["make", "-B", "spi-safe-probe"], cwd=ROOT, check=True)
...
def write_csv(rows: list[dict]) -> Path:
    ...
    path = ROOT / "build" / f"spi_safe_probe_results_{stamp}.csv"
