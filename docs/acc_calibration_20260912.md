...
- `inc/acc_calibration.h`: corrected_g = (nominal_g - bias_g) * gain.
- Applied exactly once at both startup and runtime conversion sites in `src/main.c`, before g -> m/s^2 conversion.
...
- `tools/dap_verify_acc_calibration.py`: 30s, 572 fresh coherent samples. Calibration arithmetic max error 1.172e-7 g.
