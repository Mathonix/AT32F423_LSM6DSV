/**
 * Destructive standalone SPI1/LSM6DSV matrix test.
 *
 * It first proves the sensor with GPIO Mode-3 SPI before every hardware test,
 * then tests all combinations below and stores only summaries in SRAM:
 *   Mode 0 / Mode 3
 *   8-bit frame (8-bit DT access / library-style 16-bit DT access)
 *   16-bit frame (0x8F00 / diagnostic 0x008F byte order)
 *   GPIO CS / SPI1 hardware CS
 *   stronger / moderate GPIO drive
 *   PCLK /1024, /512, /256, /128, /64, /32, /16, /8
 *   CS setup+hold 100 us, 10 us, 2 us
 *
 * 2 * 2 * 2 * 2 * 2 * 8 * 3 = 768 result rows, 100 WHO_AM_I reads each.
 */
...
static const spi_mclk_freq_div_type divs[8] = {
...
int main(void)
{
  uint32_t hwcs, drive, mode_i, frame16, variant, div_i, timing_i;
  uint16_t ref_ok;
  uint32_t config;
  static const uint8_t modes[2] = {0U, 3U};
  ...
  spi_matrix_log.result_max = SPI_MATRIX_RESULT_MAX;
  ...
  spi_matrix_log.apb2_hz = system_core_clock / 16U;
  spi_matrix_log.current_config = 0U;
  spi_matrix_log.reference_fail_groups = 0U;
  spi_matrix_log.fatal_code = 0U;

  /* Prove and reset the sensor once before the non-destructive WHO matrix. */
  ref_ok = reference_restore_and_check();
  ...
  for(hwcs = 0U; hwcs < 2U; hwcs++)
  for(drive = 0U; drive < 2U; drive++)
  for(mode_i = 0U; mode_i < 2U; mode_i++)
  for(frame16 = 0U; frame16 < 2U; frame16++)
  for(variant = 0U; variant < 2U; variant++)
  for(div_i = 0U; div_i < 8U; div_i++)
  for(timing_i = 0U; timing_i < 3U; timing_i++)
  {
    config = modes[mode_i] |
             (frame16 ? CFG_FRAME16 : 0U) |
             (variant ? CFG_VARIANT : 0U) |
             (hwcs ? CFG_HWCS : 0U) |
             (drive ? CFG_MODERATE : 0U) |
             (div_i << CFG_DIV_SHIFT) |
             (timing_i << CFG_TIMING_SHIFT);
    spi_matrix_log.current_config = config;
    run_one(config, ref_ok);
    if(spi_matrix_log.state == 3U) goto matrix_done;

    /* One slow known-good GPIO checkpoint per 24-row transaction group. */
    if((spi_matrix_log.result_count % 24U) == 0U)
    {
