...
void VQF::updateAcc(const vqf_real_t acc[3])
{
    ...
    if (q_w > vqf_real_t(1e-6)) {
        ...
        // to avoid numeric issues when acc is close to [0 0 -1], i.e. the correction step is close (<= 0.00011掳) to 180掳:
        accCorrQuat[0] = 0;
        accCorrQuat[1] = 1;
        accCorrQuat[2] = 0;
        accCorrQuat[3] = 0;
...
void VQF::setup()
{
    ...
    // the system noise increases the variance from 0 to (0.1 掳/s)^2 in biasForgettingTime seconds
