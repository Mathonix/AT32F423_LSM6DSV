/**
 * Non-destructive, directed SPI1/LSM6DSV Mode-0 comparison probe.
 *
 * Only legal WHO_AM_I reads are generated:
 *   SPI Mode 0, GPIO software CS, stronger GPIO drive
...
int main(void)
{
  uint32_t transaction, div_i, timing_i;
  uint16_t ref_ok;
  uint32_t config;
  ...
  /* Read-only precondition. Never write an IMU register in this probe. */
  ...
  for(transaction = 0U; transaction < 3U; transaction++)
  for(div_i = 0U; div_i < 8U; div_i++)
  for(timing_i = 0U; timing_i < 3U; timing_i++)
  {
    /* transaction 0: 8-bit/DT8; 1: 8-bit/DT16; 2: 16-bit/0x8F00. */
    config = 0U |
             ((transaction == 2U) ? CFG_FRAME16 : 0U) |
             ((transaction == 1U) ? CFG_VARIANT : 0U) |
             (div_i << CFG_DIV_SHIFT) |
             (timing_i << CFG_TIMING_SHIFT);
    spi_matrix_log.current_config = config;
    run_one(config, 0U);
    if(spi_matrix_log.state == 3U) goto probe_done;

    /* Prove the sensor is still alive after every single legal test row. */
    ref_ok = reference_read_count();
